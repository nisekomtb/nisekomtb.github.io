---
layout: event
categories: events
title: ツインピークス・サマーキックオフ
titleHtml: ツインピークス・<wbr>サマーキックオフ
description: 17kmの全トレイルが2026年夏に再オープン。5月31日（日）、スキルクリニック、オリエンテーションライド、フードトラック、子供向けアクティビティを、NAMBAコミュニティと一緒に楽しみましょう。
descriptionHtml: 17kmの<wbr>全<wbr>トレイルが<wbr>2026年夏に<wbr>再オープン。<wbr>5月31日<wbr>（日）、<wbr>スキルクリニック、<wbr>オリエンテーションライド、<wbr>フードトラック、<wbr>子供向けアクティビティを、<wbr>NAMBAコミュニティと<wbr>一緒に<wbr>楽しみましょう。
startDate: 2026-05-31 09:00:00 +0900
endDate: 2026-05-31 15:00:00 +0900
days: 1
time: "9:00am - 3:00pm"
location: ツインピークス・​バイクパーク、​ニセコ
address: 150, 2 Aza-150-1 Yamada, Kutchan, Abuta District, Hokkaido 044-0081
price: 0
partners:
  - name: ザ・ダーティ・デイムズ
    img: /dirty-dames.png
    url: https://www.instagram.com/dirtydamesniseko/
  - name: ザ・ナットハッチ
    img: /nuthatch.png
    url: https://www.instagram.com/thenuthatchkuromatsunai/
masthead:
  img: /assets/images/events/2026/twin-peaks-summer-kick-off/header.jpg
thumbnail: /assets/images/events/2026/twin-peaks-summer-kick-off/thumb.jpg
og:
  image: /assets/images/events/2026/twin-peaks-summer-kick-off/header.jpg
itinerary:
  days:
    - hasLocations: true
      events:
        - time: "9:00am"
          name: ダーティ・​デイムズの​事前コーヒー
          location:
            name: Rhythm Hirafu
            url: https://goo.gl/maps/78nX9Sryy9c5qXmM8
        - time: "10:00am - 11:00am"
          name: ダーティ・​デイムズ​女性スキルクリニック
          location:
            name: ツインピークス・​トレイルヘッド
            url: https://goo.gl/maps/yKza3NA7yfx5VQRx8
        - time: "10:00am - 11:00am"
          name: オリエンテーションライド
          location:
            name: ツインピークス・​トレイルヘッド
            url: https://goo.gl/maps/yKza3NA7yfx5VQRx8
        - time: "11:30am - 12:30pm"
          name: キッズ​スカベンジャーハント
          location:
            name: NAMBAテント、​トレイル​ベース
            url: https://goo.gl/maps/yKza3NA7yfx5VQRx8
        - time: "3:00pm"
          name: イベント終了
link:
  url: https://forms.gle/t3BXUgdrRnoeutrD7
  text: クリニックに​申し込む
  isSignup: true
parking: false
about: false
---

<span class="rescheduled">⛈️ <strong>日程変更：</strong>土曜日は<wbr>雨予報の<wbr>ため、<wbr>キックオフを<wbr><strong>5月31日（日）</strong>に<wbr>変更しました。<wbr>同じ<wbr>トレイル、<wbr>同じ<wbr>セッション、<wbr>より<wbr>良い<wbr>コンディションで<wbr>お待ちしています。 ⛈️</span>

雪解けから​ずっと、​トレイルクルーが​整備を​続けてきました。​ツインピークスが​帰ってきます。​スキルズパーク、​フロートレイル、​テックライン、​そして​昨年新設の​ローンチコントロールまで、​すべての​ラインが​オープン。​17km全て、​夏の​間オープンです。

1本​走って、​コミュニティを​楽しんでください。

## 朝のライド

10時に、​トレイルヘッドから​2つの​ライドが​出発します。

ダーティ・デイムズ女性スキルクリニックは、​NAMBAとの​共同開催です。​初心者から​経験豊富な​ライダーまで、​すべての​レベルの​方を​歓迎します。​9時から​Rhythm Hirafuで​コーヒーを​飲みながら、​ライド前の​集合と​バイクチェックを。​定員制で、​事前申込が​必要です。

クリニックと​並行して、​NAMBAクルーが​1時間の​オリエンテーションライドを​案内します。​ツインピークスが​初めての​方が​対象です。​ゆったりとした​ペースで、​その​後​ご自身で​楽しめるよう​パークの​レイアウトを​ご紹介します。​トレイルヘッドに​そのまま​お越しください。

## トレイルベースにて

NAMBAテントは​9時から​15時まで​オープン。​17km全ての​トレイルを​記した​新しい​トレイルヘッドマップ、​新作グッズ、​そして​ツインピークスの​今後について​お話できる​チームが​揃っています。​ザ・ナットハッチの​フードトラックも​終日​出店、​ラップの​合間の​ピットストップに​ぴったり。​11時30分から​12時30分には、​トレイルベース周辺で​キッズ向けスカベンジャーハントを​開催。​完走した​お子さま全員に​ステッカーを​プレゼントします。

## バイクを借りたい方は

当日は​Rhythmが​終日、​レンタル50%オフを​提供。​さらに​レンタルには​無料の​初心者セッションが​付きます。​トレイルヘッドに​向かう​前に、​まずは​Rhythmで​バイクを​ピックアップ。

トレイルヘッドで​お会いしましょう。
